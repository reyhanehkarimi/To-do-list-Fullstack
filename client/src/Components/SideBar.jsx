...
function SideBar() {
  ...
  const handleUpdateDirectory = (updatedName) => {
    // dispatch(addDirectory(updatedName))
    setShowModal(false);
  };
  ...
  const handleAddTask = (task, directory) => {
    setTasks((prevTasks) => [
      ...prevTasks,
      { ...task, directory: directory, id: Math.random().toString() },
    ]);
  ...
  const filteredTasks = currentDirectory
    ? tasks.filter((task) => task.directory === currentDirectory)
    : tasks;
    ...
    <div className={`sidebar ${isDarkMode ? "dark-mode" : ""}`}>
      ...
      <Nav className="flex-column p-3">
        <Button
          ...
          style={{ backgroundColor: "rgb(114, 69, 187)", border: "none" }}
        >
          Add new task
        </Button>
      ...
      <EditModal
        show={showModal && !showNewDirectoryModal}
